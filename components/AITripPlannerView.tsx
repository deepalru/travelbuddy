...
import { TripPace, TravelStyle, BudgetLevel, UserInterest } from '@/types';
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="mb-8 p-6 rounded-2xl" style={{backgroundColor: Colors.cardBackground, boxShadow: Colors.boxShadow, border: `1px solid ${Colors.cardBorder}`}}>
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fadeInUp">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-extrabold mb-2" style={{color: Colors.text}}>{t('aiTripPlannerView.mainTitle')}</h1>
        <p className="text-lg" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.mainSubtitle')}</p>
      </div>

      <Section title={t('aiTripPlannerView.theBasics')}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2" style={{color: Colors.text_secondary}}>
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl text-md focus:outline-none focus:ring-2 focus:ring-orange-500"
      ...
      <Section title={t('aiTripPlannerView.yourStyle')}>
        <div className="space-y-6">
          <div>
            <h4 className="font-semibold mb-3" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.paceTitle')}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {paceOptions.map(option => (
                <button key={option.value} onClick={() => setTripPace(option.value)} className="p-4 rounded-xl text-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500" style={{border: `2px solid ${tripPace === option.value ? Colors.primary : Colors.cardBorder}`, transform: tripPace === option.value ? 'scale(1.05)' : 'scale(1)', boxShadow: tripPace === option.value ? Colors.boxShadow : 'none'}}>
                  <div className="mx-auto mb-2" style={{color: tripPace === option.value ? Colors.primary : Colors.text_secondary}}>{option.icon}</div>
                  <p className="font-semibold text-sm" style={{color: Colors.text}}>{t(option.labelKey)}</p>
                  <p className="text-xs" style={{color: Colors.text_secondary}}>{t(option.descKey)}</p>
                </button>
              ))}
            </div>
          </div>
          <div>
            <h4 className="font-semibold mb-3" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.budgetTitle')}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
               {budgetOptions.map(option => (
                <button key={option.value} onClick={() => setTripBudget(option.value)} className="p-4 rounded-xl text-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500" style={{border: `2px solid ${tripBudget === option.value ? Colors.primary : Colors.cardBorder}`, transform: tripBudget === option.value ? 'scale(1.05)' : 'scale(1)', boxShadow: tripBudget === option.value ? Colors.boxShadow : 'none'}}>
                  <div className="mx-auto mb-2" style={{color: tripBudget === option.value ? Colors.primary : Colors.text_secondary}}>{option.icon}</div>
                  <p className="font-semibold text-sm" style={{color: Colors.text}}>{t(option.labelKey)}</p>
                  <p className="text-xs" style={{color: Colors.text_secondary}}>{t(option.descKey)}</p>
                </button>
              ))}
          ...
          <div>
            <h4 className="font-semibold mb-1" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.stylesTitle')}</h4>
             <p className="text-xs mb-3" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.stylesSubtitle')}</p>
            <div className="flex flex-wrap gap-3">
              {travelStyleOptions.map(option => {
                const isSelected = tripTravelStyles.includes(option.value);
                return (
                  <button key={option.value} onClick={() => handleStyleToggle(option.value)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-orange-500" style={{backgroundColor: isSelected ? `${Colors.primary}20` : Colors.inputBackground, color: isSelected ? Colors.primary : Colors.text_secondary, border: `1px solid ${isSelected ? Colors.primary : Colors.cardBorder}`}}>
                    {option.icon}
       ...
       <Section title={t('aiTripPlannerView.fineTune')}>
         <div>
            <h4 className="font-semibold mb-1" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.interestsTitle')}</h4>
             <p className="text-xs mb-3" style={{color: Colors.text_secondary}}>{t('aiTripPlannerView.interestsSubtitle')}</p>
             <div className="flex flex-wrap gap-2 mb-4">
                {Object.values(UserInterest).map(interest => (
                    <button type="button" key={interest} onClick={() => handleInterestTagClick(interest)} className="px-2.5 py-1 text-xs rounded-md transition-colors duration-200 hover:bg-gray-200" style={{backgroundColor: Colors.inputBackground, border: `1px solid ${Colors.cardBorder}`, color: Colors.text_secondary}}>
            ...
            <textarea
              ...
              className="w-full p-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
              style={{backgroundColor: Colors.inputBackground, border: `1px solid ${Colors.cardBorder}`}}
              rows={3}
            ></textarea>
         </div>
      ...
      <div className="mt-8 text-center">
        <button onClick={handleGenerateTripPlan} disabled={isGeneratingTripPlan || !tripDestination || !tripDuration} className="w-full max-w-sm py-4 px-6 rounded-xl font-bold text-lg text-white transition-all duration-300 disabled:opacity-60 flex items-center justify-center hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-400" style={{backgroundImage: `linear-gradient(135deg, ${Colors.primary}, ${Colors.primaryGradientEnd})`, boxShadow: Colors.boxShadowButton}}>
            {isGeneratingTripPlan ? (
                <>
