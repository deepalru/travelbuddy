...
import { Place, ItinerarySuggestion } from '@/types'; // Import ItinerarySuggestion from types.ts
...
export const ItineraryModal: React.FC<ItineraryModalProps> = ({ 
  ...
  const handleGetItineraryDirections = () => {
    ...
    const placesWithAddresses = itinerary.dailyPlan[0].activities
      .map(activity => {
        if (!activity.placeId) return null;
        const placeDetail = selectedPlaces.find(p => p.id === activity.placeId);
        return placeDetail && (placeDetail.formatted_address || placeDetail.address) 
               ? (placeDetail.formatted_address || placeDetail.address) 
               : null;
      })
      .filter(address => address !== null && address.trim() !== "") as string[];

    if (placesWithAddresses.length === 0) {
        addToast({ message: t('itineraryModal.noValidAddresses'), type: "warning"});
        return;
    }
    if (placesWithAddresses.length === 1) {
        const destination = encodeURIComponent(placesWithAddresses[0]);
    ...
    const origin = encodeURIComponent(placesWithAddresses[0]);
    const destination = encodeURIComponent(placesWithAddresses[placesWithAddresses.length - 1]);
    
    let mapsUrl = `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${destination}`;

    if (placesWithAddresses.length > 2) {
      const waypoints = placesWithAddresses.slice(1, -1).map(wp => encodeURIComponent(wp)).join('|');
