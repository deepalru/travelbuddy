...
import { CommunityPhotoUploadData } from '@/types';
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) { // 5MB limit
        setInternalError("File is too large. Maximum 5MB allowed.");
        setSelectedFile(null);
        setPreviewUrl(null);
        addToast({ message: "File is too large (max 5MB).", type: 'warning' });
        return;
      }
      if (!file.type.startsWith('image/')) {
        ...
        setSelectedFile(null);
        setPreviewUrl(null);
        addToast({ message: "Invalid file type. Please select an image.", type: 'warning' });
        return;
      }
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5">
          ...
          <div className="mt-6 flex flex-col sm:flex-row justify-end gap-3 items-center">
            <button
              ...
              className="sm:w-auto focus:ring-gray-400/70 active:scale-95"
              disabled={isLoading}
            >
              {t('close')}
            </button>
            <button
              ...
              style={{...commonButtonStyles, backgroundImage: `linear-gradient(145deg, ${Colors.primary}, ${Colors.primaryGradientEnd})`, color: 'white', width: '100%'}}
              className="sm:w-auto disabled:opacity-60 focus:ring-white/70 active:scale-95"
            >
              {isLoading ? t('communityGallery.uploadingPhoto') : t('communityGallery.uploadButton')}
            </button>
          </div>
        </form>
      </div>
