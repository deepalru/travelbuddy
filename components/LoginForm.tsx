...
const LoginForm: React.FC<LoginFormProps> = ({ onLoginSubmit, onSwitchToRegister, onGoogleLogin, isLoading, error }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6"> 
      <div>
        <label htmlFor="identifier" className="block text-sm font-medium mb-1.5" style={{ color: Colors.text_secondary }}> 
          Username or Email
        </label>
        <input
      ...
      <div>
        <label htmlFor="password" className="block text-sm font-medium mb-1.5" style={{ color: Colors.text_secondary }}> 
          Password
        </label>
        <input
      ...
      {error && (
        <p className="text-xs text-center py-2 px-3 rounded-md" style={{ color: Colors.accentError, backgroundColor: `${Colors.accentError}1A` }}>{error}</p>
      )}
