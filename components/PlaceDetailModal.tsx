...
import { Place, PlaceSummary, ExchangeRates, UserReview, CurrentUser } from '@/types'; 
import { Colors } from '@/constants';
import { askQuestionAboutPlace, fetchPlaceRecommendations } from '@/services/geminiService'; 
...
interface PlaceDetailModalProps {
  place: Place;
  onClose: () => void;
  onSelectRecommendedPlaceById: (placeId: string, isId?: boolean) => void; 
  homeCurrency?: string;
...
export const PlaceDetailModal: React.FC<PlaceDetailModalProps> = ({ place, onClose, onSelectRecommendedPlaceById, homeCurrency, exchangeRates, userReviews, onAddUserReview, currentUser, hasAccessToBasic, hasAccessToPremium }) => {
  ...
  useEffect(() => {
    if (place && hasAccessToPremium) {
      const fetchRecs = async () => {
        setIsLoadingRecommendations(true); setRecommendationsError(null); setRecommendations([]);
        addToast({ message: t('modals.findingSimilarPlaces'), type: "info", duration: 1500 });
        try {
          const recs = await fetchPlaceRecommendations(place);
          setRecommendations(recs);
          if (recs.length > 0) addToast({ message: t('modals.recommendationsLoadedSuccess'), type: "success", duration: 2000 });
        } catch (err) {
          const msg = err instanceof Error ? err.message : t('modals.recommendationsErrorGeneric');
          setRecommendationsError(msg);
          addToast({ message: t('modals.recommendationsErrorDetailed', {error: msg}), type: 'error' });
        } finally {
  ...
  }, [place, addToast, t, hasAccessToPremium]);
  ...
  const renderSection = (titleKey: string, content?: React.ReactNode, icon?: JSX.Element) => {
  ...
  return (
    <div 
      ...
      style={{ backgroundColor: 'rgba(51, 51, 51, 0.4)', backdropFilter: 'blur(5px)' }}      
      onClick={handleCloseWithAnimation}
      role="dialog" aria-modal="true" aria-labelledby="modal-title"
    >
      <div 
        className={`rounded-xl shadow-2xl overflow-hidden w-full sm:max-w-lg md:max-w-xl lg:max-w-2xl max-h-[90vh] flex flex-col relative
        ...
        <div className="overflow-y-auto flex-grow p-4 sm:p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div className="space-y-4">
                ...
                {hasAccessToPremium ? (
                    <div className="p-3 rounded-lg" style={{backgroundColor: Colors.inputBackground, border: `1px solid ${Colors.cardBorder}`}}>
                        <h4 className="font-semibold text-sm mb-1" style={{color: Colors.text}}>{t('modals.youMightAlsoLike')}</h4>
                        {isLoadingRecommendations ? <p className="text-xs" style={{color: Colors.text_secondary}}>Loading...</p> :
                        recommendationsError ? <p className="text-xs text-red-500">{recommendationsError}</p> :
                        !isLoadingRecommendations && !recommendationsError && (
                            recommendations.length > 0 ? (
                                 <ul className="space-y-2">
                                    {recommendations.map(rec => (
                                        <li key={rec.id} className="text-xs p-2 rounded-md flex items-center gap-2" style={{backgroundColor: Colors.cardBackground, border: `1px solid ${Colors.cardBorder}`}}>
                                            ...
                                            <button onClick={() => onSelectRecommendedPlaceById(rec.id, true)} className="text-white px-2 py-1 rounded-md text-[11px] font-semibold" style={{backgroundImage: `linear-gradient(135deg, ${Colors.primary}, ${Colors.primaryGradientEnd})`}}>
                                                View
                                            </button>
                                        </li>
                                    ))}
                                ...
                                <p className="text-xs italic" style={{color: Colors.text_secondary}}>{t('modals.noRecommendations')}</p>
                            )
                        )}
                    </div>
                ) : (
                    <LockedSection tier="premium" featureName="AI recommendations" />
                )}
              </div>
            </div>
        </div>
