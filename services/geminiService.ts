...
import { Place, Deal, TripPlanSuggestion, PlaceSummary, SurpriseSuggestion, TripPace, TravelStyle, BudgetLevel, HospitalInfo, UserInterest, SuggestedEmergencyNumbers, EmbassyInfo, CommunityPhoto, CommunityPhotoUploadData, ItinerarySuggestion, QuickTourPlan, SupportPoint, LocalInfo, LocalAgencyPlan } from '../types.ts';
import { GEMINI_MODEL_TEXT, LOCAL_STORAGE_COMMUNITY_PHOTOS_KEY } from '../constants.ts';
import { searchNearbyPlaces } from './placesService.ts'; // Import the new mock Places API function
...
const generateContentWithRetry = async (
  ...
  try {
    ...
    if (retries > 0) {
      ...
      // Check for 5xx Server errors or network-related RPC failures
      const isServerError = (
      ...
      if (isRateLimitError || isServerError) {
        const jitter = Math.floor(Math.random() * 1000); // Add jitter to de-synchronize retries
        const waitTime = delay + jitter;
        console.warn(`[Gemini API] Retryable error encountered (${isRateLimitError ? 'Rate limit' : 'Server/Network error'}). Retrying in ${waitTime / 1000}s... (${retries} retries left)`);
...
IMPORTANT: The entire response MUST be a single, valid JSON array of objects. Do not include any text, explanations, or conversational text. Strictly adhere to JSON formatting.
`;
};
...
- "dailyPlans": An array of objects, one for each day. Each daily plan object MUST include:
  ...
  - "photoUrl": A URL for a high-quality, relevant image for the day's theme. Construct this URL using 'https://source.unsplash.com/800x400/?' followed by URL-encoded keywords from the day's title or theme.
  - "activities": An array of activity objects. Each activity object MUST include:
...
- The "photoUrl" for each day is mandatory and must follow the Unsplash URL format.
...
- "photoUrl": A URL to a high-quality, relevant image. Construct this URL using the format 'https://source.unsplash.com/600x400/?' followed by 1-2 URL-encoded keywords from the title.
...
Example:
{
  ...
  "photoUrl": "https://source.unsplash.com/600x400/?Bolivia,Salt+Hotel",
...
- "photoUrl": A URL for a representative photo of the recommended place. Use the format 'https://source.unsplash.com/300x200/?' followed by URL-encoded keywords (e.g., Sydney+Opera+House).
...
Example of one recommendation object:
{
  ...
  "photoUrl": "https://source.unsplash.com/300x200/?Sydney+Opera+House",
  ...
  try {
    ...
    const combinedPlaces = factualPlaces.map(placeFacts => {
      ...
      const photoUrl = photoRef
          ? `https://source.unsplash.com/400x300/?${photoRef}`
          : `https://source.unsplash.com/400x300/?${encodeURIComponent(placeFacts.name || 'travel')}`;
...
export const fetchCommunityPhotos = async (): Promise<CommunityPhoto[]> => {
  ...
  return [
    { id: 'comm-photo-1', imageUrl: 'https://source.unsplash.com/600x400/?travel,sunset,bay', caption: 'Beautiful sunset over the bay!', uploaderName: 'TravelerJane', likes: 125, uploadedAt: new Date(Date.now() - 86400000).toISOString() },
    { id: 'comm-photo-2', imageUrl: 'https://source.unsplash.com/600x400/?travel,waterfall,jungle', caption: 'Found this hidden waterfall today.', uploaderName: 'AdventurerAlex', likes: 230, uploadedAt: new Date(Date.now() - 2 * 86400000).toISOString() },
    { id: 'comm-photo-3', imageUrl: 'https://source.unsplash.com/600x400/?travel,street-food,market', caption: 'Amazing street food tour.', uploaderName: 'FoodieFrank', likes: 88, uploadedAt: new Date(Date.now() - 3 * 86400000).toISOString() },
  ];
};
